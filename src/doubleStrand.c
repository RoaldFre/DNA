...
static void parseArguments(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, ":s:dt:T:N:g:c:f:rR:Fl:S:b:x:v:i:W:I:P:D:kh")) != -1)
	{
		switch (c)
		{
		case 's':
...
int main(int argc, char **argv)
{
	...
	tasks[4] = (measureTemperature ? &temperatureTask : NULL);
