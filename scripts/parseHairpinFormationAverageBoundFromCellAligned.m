% WARNING: HARDCODED FOR LOOP SIZE OF 4 (= 2 BASE PAIRS)!!!
function [timesTillZippingFromNucleation] = parseHairpinFormationAverageBoundFromCellAligned(file);
...
for run = 1:nRuns
	...
	N = numel(zippingState(1,:)) - 2; %XXX XXX HARDCODED FOR LOOPSIZE OF 4 (= 2 BASE PAIRS)
	nucleationThreshold = round(N * 0.10); % nucleation = 10% or more bound
