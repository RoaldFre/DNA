...
$main -t $timestep -T $zippingTemp -I $interval -d "$finalRelaxFile" -w "$endFile" -D $outputFile -H $requiredBounded:$nucleationBounded -M $allowedBounded -O $zippingTemp -Q $unzippingTemp -U $zippedRelaxation -X f -k -e -y $seedFile -Z $minZippingTime -g $gamma
