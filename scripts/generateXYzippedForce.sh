...
destinationDir="$destinationDirRoot/$fullSequence/temp${temperature}_gamma${gamma}_minTime${minTime}_NXY${NXY}/dt${timestep}/F${force}/N${N}"
...
$main -t $timestep -T $temperature -I $interval -K $minTime -s $seq -w $outputFile -X f -H $NXY:1 -M 10000000 -O $temperature -Q $temperature -U 0 -g $gamma -Y -E $force:0:0 # -D "/dev/null"
