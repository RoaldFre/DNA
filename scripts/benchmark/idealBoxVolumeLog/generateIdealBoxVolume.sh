...
Bfinal=$8
nRuns=$9 # Number of runs

I=$P

for B in `seq $Bmin $Bstep $Bmax` $Bfinal
do
	echo -n "$B $P $S"
	...
	do
		$main -s $seq -S $S -i m -m $sweeps -w $configfile -b $Bfinal > /dev/null
		mv $configfile ${configfile}_2
		$main -I 0.2 -P 0.2 -d ${configfile}_2 -w $configfile  -b $Bfinal -t 10 -g '5e11' > /dev/null
