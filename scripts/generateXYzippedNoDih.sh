...
# given directiory.
# During the simulation, dihedral interactions are disabled.
...
destinationDir="$destinationDirRoot/fact${factor}/$fullSequence/temp${temperature}_gamma${gamma}_minTime${minTime}_NXY${NXY}/dt${timestep}/N${N}"
...
$main -t $timestep -T $temperature -I $interval -K $minTime -s $seq -w $outputFile -X f -H $NXY:1 -M 10000000 -O $temperature -Q $temperature -U 0 -g $gamma -Y -o $factor # -D "/dev/null"
