...
function makeHairpinThresholdPlot(Ns, thresholdParams, findTauIndex, fitStartN, T, filenamePrefix, variableName, dropLogFactor, squaredDeviation, plotopt, thresholdInformationString, thresholdLegendStrings, withErrorBars, dataStartIndex, plotTauYfits, xaxis, yaxis, insetAxes, insetPosition, legendLocation)
...
if nargin < 18; insetAxes = []; end
if nargin < 19; insetPosition = [0.17 0.62 0.35 0.3]; end
if nargin < 20; legendLocation = 'southeast'; end
...
try
	additionalInfoStr = plotopt.additionalInfoStr
catch
	additionalInfoStr = ''
end
...
if squaredDeviation
	...
	observableTau = sprintf('\\sqdif{%s(\\tau)}', quantity);
else
	sqDevSuffix = '';
	sqDevStr = '';
	observable = sprintf('\\expect{%s(t)}', quantity);
	observableTau = sprintf('\\expect{%s(\\tau)}', quantity);
end
...
if numParamSets == 1
	...
	%betas = [betas, '\\'];
	resultsString = ['The results for the various values of $\lambda$ are tabulated below.\\[0.5em]',...
...
insetInfoStr = ['The horizontal axis of the log-log inset plot shows the stem length $S$ and runs from $',num2tex(theAxis(1)),'$ to $',num2tex(theAxis(2)),'$, the vertical axis corresponds to the extracted zipping times $\tau$ (the horizontal position of the points on the main plot) and runs from $',num2tex(theAxis(3)),'$ to $',num2tex(theAxis(4)),'$. '];

if plotTauYfits
	if numParamSets == 1
		mainFitString = ['The fit in the main plot has exponent $\kappa$ with $',observableTau,' \sim \tau^\kappa$. '];
	else
		mainFitString = ['The fits in the main plot have exponent $\kappa$ with $',observableTau,' \sim \tau^\kappa$. '];
	end
else
	mainFitString = '';
end
if min(Ns) < fitStartN
	fitStartNstring = [', but only data points with $S \ge ',num2str(fitStartN),'$ are used for the fits'];
else
	fitStartNstring = '';
end
if numParamSets == 1
	caption = ['Analysis of the zipping time $\tau$ as determined from simulations of ',sqDevStr,measurement,' $',quantity,'$ at $T=',num2str(T),'\degree$C. ',additionalInfoStr,mainInfoStr,insetInfoStr,mainFitString,'The fit in the inset is of the form $\tau \sim S^{\beta}$. Curves for stem lengths $S = ',NsStr,'$ are plotted',fitStartNstring,'. ',resultsString];
else
	caption = ['Analysis of the zipping time $\tau$ as determined from simulations of ',sqDevStr,measurement,' $',quantity,'$ at $T=',num2str(T),'\degree$C. ',additionalInfoStr,mainInfoStr,insetInfoStr,mainFitString,'The fits in the inset are of the form $\tau \sim S^{\beta}$. Curves for stem lengths $S = ',NsStr,'$ are plotted',fitStartNstring,'. ',resultsString];
end
