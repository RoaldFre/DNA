...
destinationDir="$destinationDirRoot/$fullSequence/g${gamma}/dt${timestep}/zipT${zippingTemp}_unzipT${unzippingTemp}_allowUnb${allowedUnboundedFraction}_nucl${nucleationFraction}/MCsweeps${monteCarloSweeps}_relaxFactor${relaxFactor}/N${N}_minZipTime${minZippingTime}"
...
$main -t $timestep -T $zippingTemp -I $interval -P $wait -d "$MCfile" -w "$finalRelaxFile" -D "/dev/null" -n -g $gamma

#just to be sure!
mkdir -p $destinationDir

echo "Starting main program!"
$main -t $timestep -T $zippingTemp -I $interval -d "$finalRelaxFile" -w "$endFile" -D $outputFile -H $requiredBounded:$nucleationBounded -M $allowedBounded -O $zippingTemp -Q $unzippingTemp -U $zippedRelaxation -X f -k -e -y $seedFile -Z $minZippingTime -g $gamma -j a


echo "Converting to native Octave format"
$scriptsdir/hairpinFormationToNativeFull.sh $outputFile
