function plotHairpinStateLowTempGamma(N, T, totTime, sweeps, wait, gammaStr, dt, dropLogFactor, DLdataStartIndex, loadFromCache, color, basedir, suff)
...
filesglob = {[dir, '/*.itf11'], [dir, '/*iks']};
dataFile = ['./data/hairpinState',suff,'T',num2str(T),'N',num2str(N),'droplog',num2str(dropLogFactor)];
if not(loadFromCache)
	[bound, time] = parseHairpinStateToArray(filesglob, false, dropLogFactor, DLdataStartIndex);
	...
	save('-binary', '-z', dataFile, 'bound', 'time');
else
